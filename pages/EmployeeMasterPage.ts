...
/**
 * Employee Master Page Object Model
 * Contains all locators and methods for Employee Master functionality
 */
export class EmployeeMasterPage extends BasePage {
  ...
  /**
   * Navigate to Masters menu and open Employee Master
   */
  async navigateToEmployeeMaster(): Promise<void> {
  ...
  /**
   * Verify employee is available in navigation
   */
  async isEmployeeMenuVisible(): Promise<boolean> {
    ...
    return await this.isElementVisible(this.employeeMenuItem);
  }
  ...
  async isLastSyncedDateDisplayed(): Promise<boolean> {
    ...
    if (!lastSyncedVisible) {
      ...
      const gridText = await this.employeeGrid.textContent();
  ...
  /**
   * Get employee count from grid
   */
  async getEmployeeCount(): Promise<number> {
    await this.employeeGrid.waitFor({ state: 'visible', timeout: 10000 });
    const employees = await this.employeeList.all();
