...
/**
 * Route Master Page Object Model
 * Contains all locators and methods for Route Master functionality
 */
export class RouteMasterPage extends BasePage {
  // Locators
  readonly mastersMenuButton: Locator;
  readonly routeMenuItem: Locator;
  readonly syncAllButton: Locator;
  readonly routeGrid: Locator;
  ...
  /**
   * Navigate to Masters menu and open Route Master
   */
  async navigateToRouteMaster(): Promise<void> {
    ...
    // Click on Route menu item
    await this.routeMenuItem.click();
  ...
  /**
   * Verify route is available in navigation
   */
  async isRouteMenuVisible(): Promise<boolean> {
    ...
    return await this.isElementVisible(this.routeMenuItem);
  }
  ...
  async isLastSyncedDateDisplayed(): Promise<boolean> {
    ...
    await this.routeGrid.waitFor({ state: 'visible', timeout: 10000 });
    ...
    if (!lastSyncedVisible) {
      ...
      const gridText = await this.routeGrid.textContent();
  ...
  /**
   * Get route count from grid
   */
  async getRouteCount(): Promise<number> {
    await this.routeGrid.waitFor({ state: 'visible', timeout: 10000 });
    const routes = await this.routeList.all();
