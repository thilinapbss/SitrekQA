...
import { WeaponMasterPage } from '../pages/WeaponMasterPage';
import * as testData from '../test-data/users.json';

test.describe('Weapon Master - Sync Functionality', () => {
    let loginPage: LoginPage;
    let weaponMasterPage: WeaponMasterPage;

    test.beforeEach(async ({ page }) => {
        loginPage = new LoginPage(page);
        weaponMasterPage = new WeaponMasterPage(page);
    ...
    test('TC_WEAPON_MASTER_001: Verify Weapon Master navigation, Sync All, and local time display', async ({ page }) => {
        console.log('\n=== TC_WEAPON_MASTER_001: Weapon Master Sync with Time Validation ===\n');

        // Navigate to Weapon Master
        await weaponMasterPage.navigateToWeaponMaster();

        // Verify grid is loaded
        const gridLoaded = await weaponMasterPage.isLocalTimeDisplayed();
        ...
        try {
            ...
            console.log('⚠ BUG: Weapon Master is missing the "Last Synced Date" column');
            throw new Error('Last Synced Date column not found in Weapon Master grid');
        }
        ...
        // Click Sync All
        await weaponMasterPage.clickSyncAll();

        // Verify local time is displayed after sync
        const gridContainer = weaponMasterPage.gridContainer;
        ...
        if (timestamps.length > 0) {
            ...
            if (hasInvalidDate) {
                ...
                throw new Error('Invalid Date found in Weapon Master grid');
            }
            ...
            throw new Error('No timestamps found in Weapon Master grid');
        }

        console.log('\n=== Test Completed ===\n');
    });

    test('TC_WEAPON_MASTER_002: Verify Sync All button updates weapon records', async ({ page }) => {
        console.log('\n=== TC_WEAPON_MASTER_002: Sync Status Validation ===\n');

        // Navigate to Weapon Master
        await weaponMasterPage.navigateToWeaponMaster();

        // Get initial count
        const initialCount = await weaponMasterPage.getWeaponCount();
        console.log(`Initial weapon count: ${initialCount}`);
        ...
        // Get updated count
        const finalCount = await weaponMasterPage.getWeaponCount();
        console.log(`Final weapon count: ${finalCount}`);
