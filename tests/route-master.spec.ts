 ...
 */
test.describe('Route Master - Verify Sync All Functionality', () => {
  let loginPage: LoginPage;
  let routeMasterPage: RouteMasterPage;

  test.beforeEach(async ({ page }) => {
    loginPage = new LoginPage(page);
    routeMasterPage = new RouteMasterPage(page);
  ...
  test('TC_ROUTE_MASTER_001: Verify Route Master navigation, sync all, and display local time', async ({ page }) => {
    // Step 1: Click Masters menu (second Masters text)
    await routeMasterPage.mastersMenuButton.click();
    await page.waitForTimeout(1000);
    console.log('✓ Clicked on Masters menu');
    
    // Step 2: Click on Route menu item
    try {
      await page.getByText('Route', { exact: true }).click({ timeout: 5000 });
    } catch {
      await page.getByText(/route/i).first().click();
    }
    await page.waitForTimeout(2000);
    console.log('✓ Clicked on Route menu item');
    
    // Step 3: Verify we're on Route Master page
    await expect(page.locator('.MuiDataGrid-root, [role="grid"], table').first()).toBeVisible({ timeout: 15000 });
    console.log('✓ Route Master page loaded with grid');
    ...
    if (hasInvalidDate) {
      ...
      console.log('⚠ Route Master may not have valid sync date data');
    }
    ...
    // Step 7: Get route count and verify routes are synced
    const routes = await page.locator('[role="row"]').all();
    const routeCount = routes.length - 1; // Exclude header row
    expect(routeCount).toBeGreaterThan(0);
    console.log(`✓ Found ${routeCount} routes in the grid`);
    console.log('✓ Routes are synced to latest');
    
    // Take screenshot for evidence
    await page.screenshot({ 
      path: `test-results/route-master-sync-success-${Date.now()}.png`,
      fullPage: true 
    });
  });

  test('TC_ROUTE_MASTER_002: Verify sync status after clicking Sync All', async ({ page }) => {
    // Step 1: Navigate to Route Master
    await routeMasterPage.mastersMenuButton.click();
    await page.waitForTimeout(1000);
    ...
    const initialRoutes = await page.locator('[role="row"]').all();
    const initialRouteCount = initialRoutes.length - 1;
    console.log(`Initial route count: ${initialRouteCount}`);
    ...
    const finalRoutes = await page.locator('[role="row"]').all();
    const finalRouteCount = finalRoutes.length - 1;
    expect(finalRouteCount).toBeGreaterThanOrEqual(initialRouteCount);
    console.log(`Final route count: ${finalRouteCount}`);
